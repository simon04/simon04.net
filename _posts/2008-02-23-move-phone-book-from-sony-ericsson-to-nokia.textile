...
To import this vCard into Thunderbird and export the contacts afterwords an additional extension is required. Download MoreFunctionsForAddressBook from the developer's "homepage":https://nic-nac-project.org/~kaosmos/morecols-en.html and set it up (Tools » Add-ons » Install)." ":https://nic-nac-project.org/~kaosmos/morecols-en.html
...
<code>mkdir new; for i in *.vcf; do cat $i| grep -v ^FN > new/$i; done</code>
